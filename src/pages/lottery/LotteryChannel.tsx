...
const LotteryChannel = ({ channelId, channelName, onBack }: Props) => {
  ...
  // Cyber circuit background (user-provided image) with cinematic depth overlays
  const sciFiBg: React.CSSProperties = {
    backgroundImage: [
      "radial-gradient(900px 500px at 50% 0%, rgba(80,140,255,0.35), transparent 60%)",
      "linear-gradient(180deg, rgba(5,10,30,0.55) 0%, rgba(5,10,30,0.75) 60%, rgba(2,5,18,0.92) 100%)",
      `url(${lotteryBg})`,
    ].join(","),
  ...
  return (
    <div className="min-h-[60vh] -mx-4 relative" style={sciFiBg}>
      <div className="absolute inset-0 pointer-events-none" style={vignette} />
      <div className="relative px-4 pt-4 pb-8 rounded-b-3xl text-cyan-50 border-b border-cyan-400/20 shadow-[0_8px_30px_-12px_rgba(80,160,255,0.5)]">
        ...
        <div className="flex gap-6 items-end">
          <button onClick={() => setTab("lottery")} className={`font-bold text-2xl tracking-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)] ${tab==="lottery" ? "text-white" : "opacity-60"}`}>
            Lottery
            {tab==="lottery" && <span className="block h-1 mt-1 bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 rounded-full shadow-[0_0_12px_rgba(80,180,255,0.9)]" />}
          </button>
          <button onClick={() => setTab("dashboard")} className={`font-bold text-2xl tracking-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)] ${tab==="dashboard" ? "text-white" : "opacity-60"}`}>
            dashboard
            {tab==="dashboard" && <span className="block h-1 mt-1 bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 rounded-full shadow-[0_0_12px_rgba(80,180,255,0.9)]" />}
          </button>
        </div>
      </div>

      <div className="relative min-h-[60vh] px-3 pt-4 pb-32 text-cyan-50">
        {tab === "lottery" ? (
          <>
            <div className="flex items-center gap-2 mb-3">
              <div className="h-4 w-4 rounded-full border-2 border-cyan-300 shadow-[0_0_8px_rgba(80,180,255,0.8)]" />
              <span className="bg-gradient-to-r from-slate-950/90 to-blue-950/80 text-cyan-100 text-sm font-bold rounded-md px-2.5 py-1 border border-cyan-400/40 shadow-[0_0_12px_rgba(80,160,255,0.4)]">Digital E lottery</span>
            </div>
